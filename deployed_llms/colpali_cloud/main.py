...
import requests
from completed.authenticate import token

# Define the URL for the SOAP service
url = 'https://affwsapi.ams360.com/v2/service.asmx'

# Your token obtained via authentication
token = token  # Replace this with the token obtained after authentication

# Ensure the token is a string before passing it to saxutils.escape
if isinstance(token, str):
    escaped_token = saxutils.escape(token)
else:
    # If token is an Element, extract the text
    escaped_token = saxutils.escape(token.text)

# Debugging: print the escaped token to see if it contains any problematic characters
print("Escaped Token:", escaped_token)
...
#Calling the changePolicyPersonnel function
changePolicyPersonnel(escaped_token)
